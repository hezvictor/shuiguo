...
DEPTH_ANYTHING_ROOT = RUNTIME_ROOT / 'Depth-Anything-V2-list3'
...
class Monster(nn.Module):
    def __init__(self, args):
        ...
        depth_ckpt = RUNTIME_ROOT / 'pretrained' / f'depth_anything_v2_{args.encoder}.pth'
        if not depth_ckpt.is_file():
            raise FileNotFoundError(f"Depth Anything checkpoint not found: {depth_ckpt}")
        state_dict_dpt = torch.load(str(depth_ckpt), map_location=map_location)
